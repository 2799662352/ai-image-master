...
describe('ApiService Seedream 5.0 Pro model config', () => {
  ...
  it('orders the model dropdown: SD5 → 腾讯 ×4 → Nano2 → 万相 → 千问 → Image 2.5 → Image2 → VIP, rest untouched after', () => {
    const keys = Object.keys(service.getAllModels())
    expect(keys.slice(0, 13)).toEqual([
      'doubao-seedream-5-0-pro-260628',
      // 四个腾讯模型相邻摆放,便于对比选择。它们是**不同的模型、不同的渠道**
      // (定价也不同),旧那条不会因为新增而下掉 —— 这里同时钉住「都在」。
