...
// 地址不落库,所以重启后素材回到「只有本地路径」的状态 —— 界面上连角标都没有,
// 用户看到的是「昨天明明传好了,今天什么都没了」。补法不是把地址存下来(试过,见
// 被回退的 8e7fb47a),而是**重新传一遍**:反正提交时本来就要传,提前到开机传等于
// 把「拖入即传」那套省等待的逻辑对重启也生效一次,顺带把打勾画回来。
//
// 范围要收紧,否则开机就是一场上传风暴:只传**看得见的那页**、且**下一步是提交**
// 的卡(草稿 / 失败 / 取消)。已出片和在飞的卡不碰 —— 它们的素材这辈子可能不会
// 再提交一次。
describe('重启后重新预传,角标回到打勾', () => {
  const BOARD_A = { id: 'b-1', name: '页1', order: 0, createdAt: 1 }
  const BOARD_B = { id: 'b-2', name: '页2', order: 1, createdAt: 2 }

  /** 摆两页,第一页是水合后的当前页(localStorage 没记忆时回第一页)。 */
  async function seedBoards(): Promise<void> {
    const db = (await import('../WorkbenchDb')).getWorkbenchDb()
    await db.putBoard(BOARD_A as never)
    await db.putBoard(BOARD_B as never)
  }

  /** 摆一张上次退出时留在库里的卡(素材只剩本地路径,地址不落库)。 */
  async function seedCard(fields: Record<string, unknown>): Promise<void> {
  ...
  it('当前页的草稿卡:水合后自动重传,转圈 → 打勾', async () => {
    const { useVideoWorkbenchStore: useStore } = await loadStore()
    await seedBoards()
    await seedCard({
      id: 'c1',
      boardId: BOARD_A.id,
      status: 'draft',
      referenceImages: [{ name: 'a', src: LOCAL_A }],
    })

    await useStore.getState().ensureHydrated()
    const imageOf = () => useStore.getState().cards.find((c) => c.id === 'c1')!.referenceImages[0]

    await vi.waitFor(() => expect(resolveRefMedia).toHaveBeenCalledWith(LOCAL_A))
    await vi.waitFor(() => expect(imageOf().uploadState).toBe('uploading'))

    deferred[0]({ ok: true, url: COS_A })

    await vi.waitFor(() => expect(imageOf().uploadState).toBe('uploaded'))
    expect(imageOf().uploadedUrl).toBe(COS_A)
    // src 始终是那条本地路径 —— 预传只是另挂地址。
    expect(imageOf().src).toBe(LOCAL_A)
  })

  it('别的页不传 —— 重启不该把整库素材重来一遍', async () => {
    const { useVideoWorkbenchStore: useStore } = await loadStore()
    await seedBoards()
    await seedCard({
      id: 'c1',
    ...
    await seedCard({
      ...
      boardId: BOARD_B.id,
      status: 'draft',
      referenceImages: [{ name: 'b', src: LOCAL_B }],
    })

    await useStore.getState().ensureHydrated()

    await vi.waitFor(() => expect(resolveRefMedia).toHaveBeenCalledWith(LOCAL_A))
    expect(resolveRefMedia).not.toHaveBeenCalledWith(LOCAL_B)
  })

  it('已出片的卡不传 —— 它的下一步不是提交', async () => {
    const { useVideoWorkbenchStore: useStore } = await loadStore()
    await seedBoards()
    await seedCard({
      id: 'c1',
  ...
  it('切到哪页传哪页;切回来不传第二遍', async () => {
    ...
    useStore.getState().switchBoard(BOARD_B.id)
    await vi.waitFor(() => expect(resolveRefMedia).toHaveBeenCalledWith(LOCAL_B))
    expect(resolveRefMedia).toHaveBeenCalledTimes(2)

    // 已有结论(打勾 / 在传 / 失败)的素材不再重发,否则来回切页就是上传风暴。
    useStore.getState().switchBoard(BOARD_A.id)
    await new Promise((r) => setTimeout(r, 10))
    expect(resolveRefMedia).toHaveBeenCalledTimes(2)
  })
})
