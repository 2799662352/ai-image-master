...
function requestOf(card: VideoWorkbenchCard): Record<string, unknown> {
  return {
    ...
    // 样片固定 480p、成片固定 1080p —— 写实际发出去的那档,不写卡片上的设置。
    resolution: card.fromDraftTaskId ? '1080p' : card.draftRun ? '480p' : card.resolution,
...
function versionRequestOf(v: VideoWorkbenchVersion): Record<string, unknown> {
  ...
  return {
    ...
    ...(s.draft ? { draft: true } : {}),
    ...(s.fromDraftTaskId ? { fromDraftTaskId: s.fromDraftTaskId } : {}),
    ratio: s.ratio,
    duration: s.duration,
