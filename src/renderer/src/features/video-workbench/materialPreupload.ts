...
//   **另挂 uploadedUrl**、src 原样留着 —— 预传没跑完 / 跑挂了 / 地址过了保鲜期被水合
//   丢掉,提交都照旧走主进程那条路。所以它是纯优化,不引入新的失败模式。
//
// 地址会跟卡片一起落库、跨重启复用(保鲜期见 `MATERIAL_UPLOAD_URL_TTL_MS`),提交
// 失败时整卡清掉(见 store 的 `withUploadCacheCleared`)——那两道兜底是敢存的前提。
//
