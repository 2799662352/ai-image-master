 ...
 *
 * 🚨 **这个函数今天没有任何生产调用方。** 全仓只有本处定义、两个测试文件、以及几份
 * 文档提到它 —— 没有定时器、没有启动钩子、没有 IPC handler 会调它。所以下面那条
 * 「账号被后台停用时连同网关 token 一起清掉」的处置**在线上不生效**:今天唯一会清掉
 * 网关 token 的路径,是用户主动点登出。
 *
 * 之所以留着而不是删掉:这段逻辑本身是对的,接上定时探测就能生效,而重写它比保留它
 * 更容易出错。但**不要把这段注释读成「有这层防护」** —— 曾经的注释就是这么写的,
 * 而文档断言一个不存在的保护,会让下一个查泄漏路径的人在这里停下。
 * 要真正启用,得单开一条改动加 60 秒定时探测(那是新增面,不是本处的注释能带过的)。
 *
...
export async function probeLiveness(): Promise<void> {
  ...
  //
  // 走 `logout()` 而不是自己 `clearCredential()` + `clearGatewayTokens()`:两套凭据必须
  // 一起清,而那条不变量的落点在 `logout()` 里(见那边)。在这里重抄一遍就等于又多一处
  // 需要记得同步的地方 —— 这一段本身就是那种遗漏的产物。
 ...
 * 登出:**两套凭据一起清**。
 *
 * 平台凭据(`credentials.ts`)与网关 token(`gatewayToken.ts`)是两套独立的东西,但
 * 「清掉前者」与「清掉后者」是**一条**不变量,不是两件可以分头记得的事。理由在网关
 * token 自身:它 `expired_time = -1`(永不过期),且被服务端四处共用 ——
 * **泄漏后无法单独吊销**(作废它会同时弄断该用户的网页端出图与项目成员判定)。
 * 所以「什么时候清掉它」就是整个平台余额方案的核心不变量。
 *
 * 从前这条不变量散在两个调用方身上(IPC 的登出 handler 一处、存活探测一处),
 * 于是下一个**直接调 `logout()`** 的人会无声地造出第三条泄漏路径 —— 没有任何编译期
 * 或测试信号会拦住他。收进这里之后,不变量由构造保证:调不到「只清一半」的登出。
 *
 * ⚠️ **落点必须是这里,不能是 `credentials.ts`。** `gatewayToken.ts` 已经 import 了
 * `credentials.ts`(它要拿平台 JWT 去换网关 token),反向再加一条边就成环 ——
 * `authBaseUrl.ts` 当初被从本文件抽出去,正是为了破同一个环。
 *
 * 必须 async 且调用方必须 await:`clearGatewayTokens()` 里压着一次 `fs.rm`,不等它就
 * 返回,调用方会立刻广播「已登出」而删盘还在半路 —— 用户此时关掉应用,进程退出会把
 * 它截断,token 原样留在盘上。
 */
