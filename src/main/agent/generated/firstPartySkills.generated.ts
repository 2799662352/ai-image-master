...
> **一板不止一张卡 = 有连续性 = \`shotlist-builder\` 自动在场**,和 \`sd2-pe\` 同一种
...
**一张卡是一次 Seedance 调用,出一段 4–15s。** 这一段里可以只有一个镜头,也可以是
一小段镜头流程 —— 那正是 \`sd2-pe\` 路径 B 的「镜头流程」,写「镜头1 / 镜头2」进同一段
是它支持的用法,不是违规。
...
反过来也别过度拆分:\`sd2-pe\` 明写「**不要**把简单动作强拆成『镜头 1 / 镜头 2』」。
...
\`sd2-pe\` 负责**怎么写**,本节只给落板前**逐卡对照的清单**。少一项就别上板。

八大要素(权威定义在 \`sd2-pe\`,缺项按它的默认策略补全并在交付时披露):
...
每一镜都要经 \`sd2-pe\` 工程化才落板 —— 把镜头表原文塞进 \`prompt\`,等于用散文去驱动
Seedance,画面十有八九不是你描述的那个。
...
- **用户手写的分镜表 / 你自己从剧本拆的镜头表 / 网上抄来的 shot list** —— 那是散文,
  **每一镜都要先过 \`sd2-pe\`** 才能进 \`prompt\`。它读起来像提示词不代表它是提示词:
  八大要素齐不齐、物理参数有没有、素材怎么绑,散文里都没有。

拿不准来源是否工程化过,就按「没过」处理 —— 多跑一次 \`sd2-pe\` 的成本远低于整板
出来不是你要的画面。
...
4. **每张卡的 prompt 都过了 \`sd2-pe\`,并对照过「一张卡的 prompt 里要有什么」。**
   不是「读起来像提示词」就算 —— 镜头表、用户手写的分镜、从剧本拆出来的描述,都是
   散文,必须工程化过才能上板。八大要素逐项过一遍,三项跨卡内容(主锚全文 / 风格光位 /
   本镜道具状态)逐卡确认在位。记 \`prompt_engineered\`。
