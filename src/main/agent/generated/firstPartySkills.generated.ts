...
>
> 少数几处**点名**了具体 skill,那是真的专属,不能换:路径 A/B 与
> seedance-cinematic-format、八大要素、create-storyboard 制片包的
> \`scenedance_prompt\` —— 都只属于 Seedance。
...
下面这份是 **Seedance 的**八大要素(权威定义在 \`sd2-pe\`,缺项按它的默认策略补全
并在交付时披露)。**\`model\` 是 \`wan3\` 时不套这一份** —— 万相有自己的结构(30 秒
六段时间轴 + 主体·场景·运动·美学控制·风格化·音效),在 wan3-cinematic-30s 里。
两套都套不是更完整,是自相矛盾。
...
路径分流只决定展开程度,不豁免交付内容。**路径 A/B 与这个结构叶子同样只属于
Seedance** —— 万相卡片不走它。
...
4. **每张卡的 prompt 都过了底座,并对照过「一张卡的 prompt 里要有什么」。**
   ...
   散文,必须工程化过才能上板。按 model 逐项过一遍底座的自检清单(Seedance 是八大
   要素,万相是六段时间轴),三项跨卡内容(主锚全文 / 风格光位 /
   本镜道具状态)逐卡确认在位 —— 这三项与模型无关。记 \`prompt_engineered\`。
