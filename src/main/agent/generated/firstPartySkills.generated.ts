...
- **画面里嵌画面(屏幕内容、照片、镜子与反射)时,里层是那一镜的主体。** 单独给它
  参考图,并在 prompt 里写明它的载体——「显示在显示器上的一张照片」「暗屏玻璃上的
  反射」。不写载体,模型会把它当成场景里真实站着的人;载体决定了它该有的画幅、眩光、
  衰减和清晰度。
...
- **一句里出现两个信息层就拆开。**「夜。老旧公寓。她盯着屏幕,屏幕上是三年前的照片」
  同时要交代环境和屏幕内容,一个镜头给不全 —— 建立镜 + 过肩镜。
- **一句里出现两个主体的两个动作就拆开**,尤其当第二个动作是情绪拐点时。
  「她伸手去碰,照片里的人转过头」压成一镜,转头那一下会被伸手稀释掉。
- **单一核心动作就是一镜**,不要为了凑数再切。
