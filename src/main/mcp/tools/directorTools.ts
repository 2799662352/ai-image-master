...
ANIMATION (select an advanced mannequin first): search_animations {keyword?, category?, limit?} (≈2000-clip Mixamo catalog; returns urls) | play_animation {url, name?, ext?} (url accepts https OR a LOCAL fbx/glb/json animation file path — import the user's own animation, or replay a .glb from export_pose_clip_glb) | pause_animation | resume_animation | stop_animation | seek_animation {sec}
