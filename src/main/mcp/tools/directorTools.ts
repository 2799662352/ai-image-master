...
ANIMATION (select an advanced mannequin first): search_animations {keyword?, category?, limit?} (≈2000-clip Mixamo catalog; returns urls) | play_animation {url, name?, ext?} (url accepts https OR a LOCAL fbx/glb/json/vmd animation file path — import the user's own animation, replay a .glb from export_pose_clip_glb, or play an MMD .vmd motion on a selected MMD model) | pause_animation | resume_animation | stop_animation | seek_animation {sec}
...
enter | exit — toggle recording layout. add_keyframe {t: seconds} — capture current camera as a keyframe at time t (move the camera between calls: director_exec or apply_camera_slot / set_transform). import_camera_clip {url, ext?, mode?: 'replace'|'append', atSec?} — load a camera-move file straight into the timeline (json = director-camera@1, vmd = MMD camera motion, glb/gltf/fbx = baked camera node; url accepts https OR a LOCAL file path — no need to serve files over HTTP). list | remove {id} | clear | seek {t} — manage/scrub keyframes. play {durationSec?} — preview the interpolated camera move once (no export; needs ≥2 keyframes). export {durationSec?, resolution?: '1080p'|'2k'|'4k', fps?: 24|30|60, quality?: 'low'|'medium'|'high'|'max'} — interpolate keyframes into a camera move, record the canvas and save a video file; returns \`videoPath\`. capture_video {durationSec?≤60, resolution?, fps?, quality?} — record the LIVE viewport as-is (no keyframe interpolation; great for recording a playing character animation); returns \`videoPath\`.`,
    inputSchema: z.object({
      action: z.enum(['enter', 'exit', 'add_keyframe', 'import_camera_clip', 'list', 'remove', 'clear', 'seek', 'play', 'export', 'capture_video']),
      t: z.number().optional(),
      id: z.string().optional(),
      url: z.string().optional().describe('import_camera_clip: camera file (https or local path; json/vmd/glb/gltf/fbx).'),
      ext: z.string().optional().describe('import_camera_clip: explicit extension when the url lacks one.'),
      mode: z.enum(['replace', 'append']).optional().describe('import_camera_clip: replace timeline (default) or append after last keyframe.'),
      atSec: z.number().optional().describe('import_camera_clip: shift imported keys to start at this time.'),
