...
export interface VideoWorkbenchMaterial {
   ...
   *
   * 会随卡片落库,重启后接着用（配 `uploadedAt` 做保鲜期,见下）。地址本身是
   * 桶内对象的公开 URL、不带签名有效期,所以「过期」这件事只可能来自两头:
   * 本地文件被改过（地址还活着但内容对不上）、或对象被删。两者都无法从渲染层
   * 判断,只能靠保鲜期缩短暴露窗口 + 提交失败时清缓存自愈。
   *
  ...
  uploadedUrl?: string
  /**
   * `uploadedUrl` 拿到的时刻。水合时用它算保鲜期:超过
   * {@link MATERIAL_UPLOAD_URL_TTL_MS} 的地址直接丢掉,退回「提交时现传」。
   * 缺这个字段的（更早版本落的库）一律按过期处理 —— 无从判断新旧就别赌。
   */
  uploadedAt?: number
  /**
   * 预传的进行态,**只为界面存在**(转圈 / 打勾 / 传失败),不落库:重启后由
   * `uploadedUrl` 在不在重新算出来。
   *
...
/**
 * 预传地址的保鲜期。超过这个岁数的 `uploadedUrl` 在水合时丢弃,退回「提交时
 * 现传」——不是因为地址会到期（桶内对象 URL 不带签名),而是因为本地文件可能
 * 已经被改过,而渲染层没有便宜的办法核对。七天是权衡:够长到覆盖「今天拖进来、
 * 过几天才生成」的常见节奏,又短到不至于让一个改过的文件一直命中旧地址。
 */
export const MATERIAL_UPLOAD_URL_TTL_MS = 7 * 24 * 60 * 60 * 1000
