...
export interface VideoWorkbenchMaterial {
   ...
   * 上传）远小于代价（拿一个死链去提交,而且重试还是同一个死链）。
   *
   * 省下的那次上传由「重启后重新预传」补回来:水合时对当前页可提交的卡重发一遍
   * (`store.ts` 的 `resumePreuploadsForBoard`),拿到的是当场 mint 的新地址,没有
   * 死链问题。真没传成也不影响出片 —— 提交时主进程照旧从磁盘流式上传。
   *
