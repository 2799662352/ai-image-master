...
| `FilesystemBackend` | Local disk (use `virtual_mode=True`) |
| `StoreBackend` | Cross-thread persistent (requires `store`) |
| `CompositeBackend` | Route paths to different backends |
| `LocalShellBackend` | Filesystem + `execute` shell tool (dev only) |
| Sandbox backends | Isolated execution (Modal, Daytona, Deno) |
</backend-options>
...
<skills-agent-flow>
When skills are configured, a "Skills System" section is injected into the agent's system prompt. The agent follows a three-step process:

1. **Match** — When a user prompt arrives, the agent checks skill descriptions to find relevant ones.
2. **Read** — If a skill applies, the agent uses `read_file` to load the full SKILL.md content.
3. **Execute** — The agent follows the skill's instructions and accesses supporting files as needed.

Write clear, specific descriptions in SKILL.md frontmatter — the agent decides whether to use a skill based on the description alone.
</skills-agent-flow>
...
<ex-skills-with-statebackend>
<python>
Seed skill files into StateBackend via invoke(). Use create_file_data() to format content.
```python
from deepagents import create_deep_agent
from deepagents.backends.utils import create_file_data
from langgraph.checkpoint.memory import MemorySaver
...
result = agent.invoke(
    {
        "messages": [{"role": "user", "content": "Write tests for my module"}],
        "files": skills_files  # Seed virtual filesystem
    },
...
- Model via string format: `"provider:model"` (e.g., `"openai:gpt-4o"`, `"anthropic:claude-sonnet-4-6"`)
...
<fix-skill-source-precedence>
When multiple skill sources contain a skill with the same name, the last source wins.
```python
# If both sources contain "web-search", the one from "/skills/project/" wins.
agent = create_deep_agent(
    skills=["/skills/user/", "/skills/project/"],  # Last wins
    ...
)
```
</fix-skill-source-precedence>

<fix-skill-size-limits>
SKILL.md files must be under 10 MB — larger files are skipped. The `description` field is truncated to 1024 characters.
</fix-skill-size-limits>
