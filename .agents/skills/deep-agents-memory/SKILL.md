...
The `read_file` tool natively supports image files (`.png`, `.jpg`, `.jpeg`, `.gif`, `.webp`) across all backends, returning them as multimodal content blocks.
</overview>
...
| Dev with shell execution | LocalShellBackend | Filesystem + execute tool |
| Isolated code execution | Sandbox backends | Modal, Daytona, Deno |
...
<ex-localshell-backend>
<python>
LocalShellBackend extends FilesystemBackend with an execute tool for shell commands. Dev only — never use in production.
```python
from deepagents import create_deep_agent
...
Security risks: commands run directly on host with no sandboxing. Use Sandbox backends for production.
</python>
</ex-localshell-backend>
...
- LocalShellBackend: virtual_mode provides no security with shell access (commands can reach any path)
</boundaries>
