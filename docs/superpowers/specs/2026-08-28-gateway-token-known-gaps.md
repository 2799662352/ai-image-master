...
## 零、唯一会清掉这枚 token 的路径是「用户主动点登出」

**这一节先于其余各节，因为它纠正的是本文档此前默认成立、实际不成立的一个前提。**

`session.ts` 里的 `probeLiveness()`（封号 / 踢下线探测，401/403 时连同网关 token 一起清）**在全仓没有任何生产调用方**——只有函数定义、两个测试文件、以及几份文档提到它。没有定时器、没有启动钩子、没有 IPC handler 会调它。

所以：

- ✅ **生效**：用户在设置页点「退出登录」→ `session.logout()` → 平台凭据与网关 token 一起清、加密文件删盘。
- ❌ **不生效**：账号在后台被停用 / 被踢下线。桌面端不会察觉，那枚 token 留在内存与盘上，下次启动 `loadPersisted()` 还会读回来，被停用的账号**仍能接着花平台余额**，直到用户自己想起来点一次登出。

同样的错误声明在 `../specs/2026-08-25-desktop-browser-login-design.md` 也有一处（把「60 秒存活探测」列为永久 JWT 的三点补偿之一），已一并改正。

**为什么单独写出来而不是顺手接上定时探测**：接探测是新增面（要定时器、要决定在哪个生命周期挂、要处理离线抖动），该单开一条改动。而在那之前，**文档必须停止断言一个不存在的保护**——上一版的 `session.ts` 注释正是这么写的，它会让下一个来查「token 什么时候会被清掉」的人在那里停下、以为已经覆盖了停用场景。

不变量本身已经收进 `session.logout()`（不再散在 IPC handler 与探测两处各自记得清），所以将来无论谁接上探测、还是新增别的登出入口，都不会再漏掉网关 token 那一半。
