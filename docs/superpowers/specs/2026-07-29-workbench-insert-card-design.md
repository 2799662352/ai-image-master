...
上限：**沿用 `addCards` 今天的 `evict()` 语义**，不新增硬拒。`WORKBENCH_MAX_CARDS = 200` 是防素材
`data:` URL 撑爆 IndexedDB 的软上限，超出时自动淘汰最旧的**终态**卡；`addCards` 今天就不检查它。
让插入抛错而追加淘汰会造出两种脾气，也直接违反「不传锚点时行为与今天逐字一致」。

**连带修一个既有缺陷（用户 2026-07-29 拍板）**：`apply` 原本在超限时整体拒绝，用户要求拆掉。
但拆之前必须先修 `evict()` —— 它只删数据库并返回被删 id，而唯一调用点 `store.ts:860` 是
`void getWorkbenchDb().evict()`，**返回值被丢弃、内存里的卡还在**，于是超限时界面正常、重启后
凭空少卡。`apply` 那条拒绝正是在挡这个缺陷的规模化版本（一次加 100 张 = 重启后蒸发 100 张）。
修法：消费返回值、同步从内存移除、弹一次 toast 说明淘汰了几张，然后才拆掉 apply 的拒绝。
详见实施计划 Task 0。
