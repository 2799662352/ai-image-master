...
- 不改上传链路（已是最佳实践）。
- 不在 Phase 1 改 `fetchImageBytes`。**但这条判断在 2026-08-01 被推翻了，改为独立的 Phase 1.5**，见文末「Phase 1.5」一节——原本的理由「图片体积小，全量 buffer 的内存代价可接受」是错的。
...
> **增补记录（2026-08-01）**：本节是对原设计「不改 `fetchImageBytes`」这一非目标的推翻。原理由是「图片体积小，全量 buffer 的内存代价可接受」——这个判断错了，而反证就在我们自己的代码注释里。

### 推翻的依据

`src/main/index.ts:1874` 有一段 P0 事故的记录：
...
也就是说，**图片 buffer 撑爆主进程这件事已经真实发生过一次**，量级是 30MB+ 而非「小」。当时的修法是加并发闸门 `MAX_CONCURRENT_UPLOADS_MAIN = 4`，把 fetch 与 base64 解码一起圈进去。

那是止血不是根治：峰值内存仍是 `4 × 30MB × 2`（chunks 数组与 concat 出的新 Buffer 同时存在）约 240MB，而且代价是**把上传串行化到 4 路**，批量出图时吞吐被这个闸门卡死。

### 目标设计

改造面比 Phase 1 小，因为调用方本来就要落盘。`cos:enqueue-upload-from-url` 拿到字节后只做两件事：`saveGeneratedImageLocally` 写本地副本、`uploadBufferToBucket` 传 COS。两件都可以从文件走：

- `fetchImageBytes` 增加流式变体（或加 `destPath` 参数），直接写到本地副本该在的位置
- COS 转存改用**已存在的** `uploadStreamToBucket`（`sliceUploadFile` 分片），不新造轮子
- 中间那份 Buffer 彻底消失

**收益兑现点是重新评估 `MAX_CONCURRENT_UPLOADS_MAIN`。** 这个闸门存在的唯一理由就是防 buffer 堆积；根因消失后它应该能放宽或去掉，批量出图的吞吐才真正回来。改造完必须回头动它，否则这次重构只省了内存、没换来速度。

### 两个必须保住的东西

**`data:` URL 分支不改。** base64 解码天然产出 Buffer，此时字节已在内存里，流式没有意义。它已有 80MB 的 IPC 闸门（`rejectOversizedBase64`）挡着。

**`fetchImageBytes` 的错误分类一条都不能丢。** 它是全仓库做得最细的一处——403/404 立刻放弃、408/429/5xx 才重试、空响应体单独判失败。好在这套判断在响应头阶段就完成了，与 body 如何消费解耦，所以流式化不会动到它。改造时要有测试钉住这一点。
