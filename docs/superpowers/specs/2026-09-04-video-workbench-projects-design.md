...
- 每部剧至少一个分段:新建剧自带「分段 1」,不存在零分段的空态。(实施时的偏离,2026-09-06:
  store 里 `activeBoardId: string` 是硬不变量,`addCards` 与 MCP `add_tasks` 都依赖它,允许零分段要给
  整个 store 加可空分支,只换来一张空态插画。)
...
4. ~~toast「已导出 · 在文件夹中显示」~~ → **确认页成功态**:「已导出到 <path>」+「在文件夹中显示」+「完成」
   (实施时的偏离,2026-09-07:现有 `ToastItem` 没有 action 槽,为一个按钮给全站 toast 加字段不值;
   留在确认页里用户反而更清楚文件去了哪)。
5. 失败:离线/上传失败 → 明确文案(点名是哪个素材)留在确认页,可重试;不写半个文件。

### 6.4 导入流程

1. 剧栏底部 / 总览右上「导入工程」→ 系统打开对话框(过滤 `*.catwb.json`);或把文件拖到剧栏。
2. 主进程读文件(stat 先看体积,50 MB 闸;只接受 `.catwb.json`)→ **渲染端**解析加固(拒绝 `__proto__` /
   `constructor` / `prototype` 键;字段校验、未知字段放行;版本门)→ 摘要。
   (实施时的偏离,2026-09-07:解析放渲染端 `projectFile.ts`,与导出共用同一份纯函数与单测,
   不在主进程写第二份校验;主进程只守文件系统那一层。)
...
**实施时的收窄(2026-09-07):** 仓库里已有 `materialTransfer.ts` —— 粘贴的 `data:image/*` 在挂上卡片
时就经字节通道转存成 COS 地址,联网时几秒内完成;所以本节的「落文件 + `workbench-assets` 目录 + GC」
没有做,只补了「水合后空闲时扫全部卡片,对残留的 `data:image/*`(离线粘贴 / 转存失败 / 老数据)重新
发起转存」(`store.ts` 的 `sweepInlineMaterials`),幂等、失败保持原样下次再试。内联视频/音频仍留原样:
字节通道只认 image/*,导出时遇到会明确报错。若将来要离线可用的本地副本,再按本节原文另开计划。
