...
**源代码中不存储任何明文密钥。** 所有敏感凭据通过以下方式管理：

### 腾讯云 COS 凭据（宫格拆图功能）

凭据加载优先级（`config.ts` → `getCredentials()`）：

| 优先级 | 来源 | 说明 |
|--------|------|------|
| 1 | 用户在应用内配置 | 通过 electron-store 加密存储（`tencent-credentials`） |
| 2 | 环境变量 | `COS_SECRET_ID` / `COS_SECRET_KEY` / `COS_BUCKET` / `COS_REGION` |
| 3 | 内置凭据文件 | `cos-credentials.json`（打包在 `resources/` 中） |

### 文件结构

```
项目根目录/
├── .env                    ← COS 上传脚本用（gitignored）
├── cos-credentials.json    ← 应用内置凭据（gitignored，打包时带入）
└── .gitignore              ← 确保以上两个文件不被提交
```

### 安全规则

- `.env` 和 `cos-credentials.json` 已加入 `.gitignore`，**绝不可提交到 Git**
- `electron-builder.yml` 的 `extraResources` 会在打包时把 `cos-credentials.json` 复制到安装包的 `resources/` 目录
- 开发模式下从项目根目录读取 `cos-credentials.json`
- **git 历史中残留的密钥**：旧 commit 中仍包含明文密钥，如果仓库是 public，建议到腾讯云控制台轮换 SecretId/SecretKey 使旧密钥失效

### 新开发者设置

1. 从团队获取 `cos-credentials.json` 和 `.env` 文件（不在仓库中）
2. 放置到项目根目录
3. 格式参考：

**cos-credentials.json：**
```json
{
  "secretId": "你的SecretId",
