...
> **★ sd2-pe 衍生模块:** 本 skill 是 `sd2-pe`(Seedance 2.0 提示词工程 · 总兜底/起点)的**下游衍生**。所有视频提示词的结构骨架——八大核心要素、A/B 复杂度路径、四个强制兜底包、`@图片N` / `<主体N>@图片N` 引用语法、资产 ID 脱敏、一镜一运动——**一律以 `sd2-pe` 为准**。本 skill 只在 `sd2-pe` 产出的工程化 prompt 之上,叠加「模型对齐 + 出片编排 + 爆款体检」。流程:**先过 `sd2-pe` 搭好结构,再进本 skill 做出片**。

把 **Seedance 2.0 满血版** 当成它论文里真实的样子来用,而不是当成一个「随便写句话就出片」的黑盒。本 skill 是「模型对齐 + 出片编排 + 爆款体检」这一层,坐在 `sd2-pe` 与已有的 `storyboard-*` / `director-*` 工艺型 skill 之上。
