...
> **正文来源**:火山方舟官方 `sd25-pe` v0.1.0,2026-08-08 取自 arkdocs skills 源。
> 从下一节「目的」起到文末是官方原文,**一字未改**。2.5 的模板结构、素材职责、
> 自检清单以官方为准;与本节冲突时听官方的。
>
> 本节只补官方不可能知道的四件事:什么时候自动加载、和 2.0 那套怎么分工、
> 运镜知识库的硬性纪律、以及「参数不写进 Prompt」在本 app 具体落到哪个控件。
...
## 与 sd2-pe 同级:两个底座一起被路由带出来

`sd2-pe` 与本 skill 是**同级的提示词底座**,不是二选一的替代关系。视频提示词任务
由入口把两个一起带出来,各管一层:

| 这一层 | 归谁 |
|---|---|
| 引用语法、八大要素、镜头分镜的通用工程写法、运镜措辞 | `sd2-pe` |
| 2.5 专属:编辑 / 延长 / 关键帧 / 宫格分镜 / 白模、素材逐份职责与主体映射、30 秒时间段 | **本 skill** |

**默认按 2.5 写。** 本 app 的视频链路以 Seedance 2.5 为准 —— 它是唯一支持
`taskMode` 编辑 / 延长、30 秒时长和 50 份素材的档位,拿不准模型时按它写不会亏。
只有当卡片 / 请求的 model **明确**是 2.0 家族(`2.0` / `2.0-fast` / `2.0-mini`)时,
才改用 2.0 的八大要素公式与 15 秒上限,本 skill 里 2.5 专属的编辑 / 延长 / 30 秒
段落此时不适用。

**冲突时的裁决顺序:** 2.5 任务上,本 skill 的模板结构优先(它是官方对 2.5 的定义);
两边不冲突的通用写法照 `sd2-pe`。**同级不等于同时套模板** —— 套模板只套一套,
别把 2.0 的公式和 2.5 的模板揉进同一条 Prompt,那会产出既不满足公式、也没走完
素材职责流程的四不像。

## 复杂任务继续往下走:结构叶子

复杂、多镜、混合媒介,或需要展开导演 / 作品参考 —— 任一命中就继续加载
`seedance-cinematic-format`(路径 B 的结构叶子),与 2.0 那条路走法一致。
简单单镜由本底座独立完成,不必加载。

## 运镜知识库:按需查,不按条数查

**查的单位是「运镜动作」,不是「提示词条数」。** 这条曾经写成「每条提示词至少三次」——
一板 20 个镜头就是 60 次工具往返,几十分钟耗在这儿,而其中绝大多数是把同一个
`dolly in` 反复查。改成:

- **只有这条镜头用了非常规运动**(dolly / arc / crane / whip pan / rack focus / dolly zoom…)
  才查 `search_cinematography_kb`。固定机位、常规景别(medium shot, eye-level)**不查** ——
  这些词本来就没有歧义,查了也只是拿回同一句话。
- **一个动作查一次就够。** 拿到权威写法就落字,不必凑「术语 / 描述规范 / 修正对」三个面。
  只有这个动作是本片的关键调度、或者第一版措辞被判定为含糊时,才追加一次查范例或
  critique/fix 对照。
- **同一任务内查过的动作直接复用结论,不重查。** 一个项目通常只有 2–4 个不同运动,
  所以整片的查库次数是个位数,不是镜头数的三倍。
- **查到什么就原样写什么,不要译成中文。** `dolly in`、`rack focus`、`truck left`、
  `deep focus`、`smears`、`impact_frames` 是模型训练时见到的**确切词形**,技法标签
  更是 sakugabooru 的数据集标识符。中文对译(「推镜」「变焦对焦」「残影」)不是等价物 ——
  换过去等于把精确坐标换成大致方向,而标签类的中译**根本指不到任何东西**。
  **提示词整体是中文也照样保留这些英文原词**:中英混排正是知识库里那些参考 caption
  的真实样子,不是将就。跟用户解释时怎么说都行,但进提示词的那份必须是检索回来的原文。
- 做动画风格时按同样口径用 `query_sakuga_dataset` 拿技法标签(smears / impact_frames /
  background_animation…)—— 那些是可核实的名词,比「作画精良」能进提示词。

> ⚠️ 官方原文末尾那条 `No network access` 是给**无网运行环境**的降级说明,不是禁止查库。
> 本 app 装着知识库 MCP,所以上面这些「该查的时候」要真查;工具不可用 / 未配 key 时
> 退回联网检索,并在交付里注明该条未经库校准。
...
官方正文用的 `@图片N` / `@视频N` / `@音频N` 与本 app 的写作形式一致,可以照抄。
运行时会统一归一为 `图片N` / `视频N` / `音频N` 再发给上游 —— **`@` 是写作助记,
不是上游 API 参数**,不要向用户这么描述。不带 `@` 的 `图片N`、`<主体N>(参考 图片N)`、
`【@图片N】` 等写法运行时也会自动归一,不必手改用户输入。
