# catimation-storyboard 家族目录(按症状挑技法)

本目录列出插件内全部 29 个 storyboard-* 工艺技能。用法:先判断这次的画面/提示词问题属于 A–G 哪一类(常常跨多类),再逐个加载对应子技能(读其 SKILL.md / references),用它的真实技法词填字段——别凭记忆。宁可多加载一个,也不要凭记忆硬写;加载后发现不合适,丢掉即可。

> 写多镜/导演级镜头时,优先回到 `director-orchestrator` 走 STEP 0 反问,它会把 director-* 与 storyboard-* 串成一条流水线;本目录用于"我已知要用分镜技法,想快速找到对的那几个"。
...
1. 提示词产出走**结构化文本(绝不 JSON)**,物理可复现参数优先于情绪形容词;默认只写正向。
2. 写好后:图像交 `catimation-image`,视频交 `catimation-video`;多镜/连续性回 `director-orchestrator` 统筹。
3. 不确定的技法词先联网查证再落笔。
