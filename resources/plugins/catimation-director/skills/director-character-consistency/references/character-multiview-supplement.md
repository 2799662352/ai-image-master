...
>
> **定位(锚点纪律):** 三视图 / 四视图 / 多视图角色板可以按用户需要直接作为
> `identity-hard` 主锚，也可以只作设计补充。用户已指定就采用；若同时存在多视图板
> 与大头照+全身照等多套候选、主锚不明确时先询问用户。
...
- 用户没有多角度需求且未指定时，可用更轻量的大头照+全身照，不机械生成多视图。
...
- 多视图是**资产生产工具**；用户选它作为主锚时，最终落地到分镜/视频也以该
  多视图资产的人像库句柄为准。
