...
1. 提示词写好 → **交回上游入口执行生成**:图像由 catimation-image 调
   ...
   Skill 写作中的素材一律按 `@图片1/@视频1/@音频1` 引用，交给生成工具时由
   运行时归一为无 `@` 形式。本技能不直接调生成工具。
2. **出片自检由上游入口的分级 QA 负责**(快速/视觉/内容/发布四级)。本技能只
   在重生成时提供「哪个技法的哪个字段没落地」的针对性改进点。
3. 视频提示词底座始终是 `sd2-pe`(八大要素 + 多模态绑定 + 12 项内容 +
   五大必备块)。13 维镜头设计属于路径 B,加载 `seedance-cinematic-format`;
   13 维技法结论写进对应内容要素,标题、分段与散文形式自由,不另起一套冲突格式。
