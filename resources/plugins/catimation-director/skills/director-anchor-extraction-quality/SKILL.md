...
- 不用:锚点已定、只需跨镜复用一致性(→ `director-character-consistency`)。
...
**何时改用 `director-character-consistency`:** 锚点已提取确定、需要**跨镜复用与一致性校验**(发型/服装/道具不变)时,用 character-consistency;本 skill 只负责**从参考图提取锚点的质量**。
