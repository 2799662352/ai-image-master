...
把剧本变成 SceneDance/Seedance 可用的制片包,产出表现得像由导演、分镜师、剪辑师、AI 视频协调员共同准备。**输出不是画面描述清单,而是一份连续性受控的出片计划**——用于生成许多 `0-15s` 视频片段并能顺畅剪到一起。
...
- SceneDance/Seedance 片段必须 `<= 15s`。
- 新项目默认 `SH### = CLIP###`:一个电影镜头 = 一次 SceneDance 视频生成。仅低风险插入镜或用户明确要求时,一个 `CLIP###` 才覆盖多镜。
- 用户说"分镜图""重新生成分镜图""storyboard images""SceneDance inputs"默认是**逐 clip 交付**。除非用户明确要总览/拼贴图,不要用一张全片总览板搪塞。
- 全片总览板/主分镜表/拼贴图只用于 review,不是最终 SceneDance 分镜图,不计入已交付的 `CLIP###` 分镜产出。
...
- 时长由剧情驱动。不要按固定总量(如一分钟 = 4×15s)均分。插入/反应 `2-4s`、清晰物理动作 `4-7s`、持续表演或氛围 `8-12s`、稳定长镜 `12-15s`。
...
- 角色锚点默认用单锚点人像库:大头照(正脸无表情)+ 全身照;三视图/四视图可作可选补充,慎用(多视图易触发 ID 漂移与双胞胎)。`character_bible.md` 按此约束记录锚点。
...
- 若两相邻镜因总时长 `<=15s` 合并,它们在 `clip_plan.md` 成为一个明确 `CLIP###`。为该合并 clip 生成一张干净起始关键帧和一张单 clip 生产分镜板;不要把双格/多镜板当主图喂 SceneDance。
...
4. 建资产计划:角色转身、表情表、姿势表、场景建立/反打、道具/产品表、干净起始关键帧、关键动作帧、出点帧、可选桥接帧、最终 clip 分镜板。
5. 建 shot cards,默认每 `CLIP###` 一张 `SH###`。每镜按动作负载、情绪、信息密度、剪辑节奏、接力需求选时长。
6. 写参考输入矩阵。每个 SceneDance 镜必须列出主干净输入图 + 所有辅助角色/场景/道具/分镜参考。
...
10. 要求生成图像时先生成/准备参考图。先出干净 panel/关键帧,再为每个 `CLIP###` 拼装一张确定性生产分镜板。不要把 AI 生成板版式当最终产物,除非明确要求否则不出全片总览板。
...
13. 校验:每 clip `<=15s`、每镜有衔接与边界计划、每镜有主输入图 ID、每张承诺图被追踪、无遗漏的身份/空间/风格锁定。
...
完整制片包必须包含:脚本分析与项目简报;角色、场景、道具/产品、风格、连贯性圣经;资产生成清单;主分镜与详细 shot cards;SceneDance 参考输入矩阵;衔接设计矩阵;剪辑边界矩阵;Image 2 中文与英文提示词;SceneDance 镜头提示词;SceneDance 使用清单;后期/剪映/CapCut 计划;风险与回退计划;最终图清单。
...
- 视频提示词最终经 `sd2-pe` 工程化(八大要素 + 多模态绑定 + 必载结构叶子的 12 字段骨架输出)后再交生成工具;每段 Seedance ≤15s。
