...
2. 加载 **catimation-image**,按其规则出图:
   ...
   - 渠道:**省略 `model` 即跟随用户在 chat composer 选的渠道(默认 VIP)**;仅在有明确理由时覆盖 `model` —— 组图系列(count>1)→ `wan2.7-image-pro`,或用户点名的渠道(`custom-imagemodel-gt` 腾讯 image2 / `gemini-3.1-flash-image` Nano2)。
