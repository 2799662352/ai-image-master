...
2. **镜头规划（所有任务统一高规格，没有"简单视频"降级路径）**：
   - **编辑 / 延长 / 组合**：保留 `严格编辑 @视频N` / `向前/向后延长 @视频N` / 组合句式作为任务主句，通常为单镜；**交付规格与其它任务完全相同**，不因单点操作缩水。
...
2. **长图 / 九宫格拆分（grid 是好素材，但 Seedance 单图参考要拆）**：九宫格 / 长图（ffmpeg contact sheet、storyboard 网格、按剧情裁剪拼接的分镜板）都是优质素材来源——但 Seedance 的单张参考**无法当多帧解析**，所以**先把它拆成单图、再分别绑 `@图片N`**，而不是丢弃 grid 或整张塞进去。
3. **多视图素材检测**：若上传人物三视图 / 四视图，**默认建议** 拆分为 大头照（仅头部、无表情）+ 全身照；三视图 / 四视图仅作可选补充，慎用（多视图易触发 ID 漂移与双胞胎）。
...
> 12 字段骨架交付：
> - **编辑 / 延长 / 组合** 保留 `严格编辑 @视频N` / `向前/向后延长 @视频N` / 组合句式作为任务主句，放在 `[任务与创作意图]` 与 `[动作流程与镜头设计]` 的开头。
...
**第二段：镜头分镜（仅使用多模态参考形态）**
- 顺序使用 `镜头1 / 镜头2 / 镜头3 …`，**禁止写绝对秒数**（如 `0–3s`）。Seedance 2.0 对精确时间支持不稳定。单镜任务只写 `镜头1`，同样按四要素组织。
...
- **人脸参考最佳实践**：默认用 大头照 + 全身照单锚点，**三视图 / 四视图可作可选补充，慎用**（多视图易触发 ID 漂移与双胞胎）。
