...
> **写运镜 / 分镜之前先查知识库(硬性,每条提示词至少三次):** 落笔"镜头如何运动 / 镜头顺序 / 运镜"这类字段前,**调 `search_cinematography_kb` 至少三次**查「运镜与结构化描述库」(阿里百炼 RAG),三次各查一个面,别用同义词把同一个问题问三遍:①**术语**——你要用的那个运动本身(`dolly in`/`arc`/`whip pan`/`rack focus`…)的权威写法;②**描述规范**——它在结构化分镜描述里该怎么落字(CHAI 五维:camera / motion / scene / spatial / subject);③**范例或修正对**——同类专业 caption,或一条 critique/fix 对照着改措辞,修正对尤其值钱,它直接指出这类描述通常错在哪。做动画风格时再加一次 `query_sakuga_dataset`,拿真实技法标签(smears / impact_frames / background_animation…)与作画/studio 归属,那些是可核实的名词,比"作画精良"能进提示词。同一个运动在同一任务里查过就可复用结论,**换了运动必须重查**;工具不可用 / 未配 key 时退回联网检索,并在交付里说明该条未经库校准。
