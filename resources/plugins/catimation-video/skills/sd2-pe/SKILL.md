...
2. **复杂度判定（仅对多模态参考做）**：
   ...
   - **核心观点**：上文"任务分类"的推荐句式是 **语句工具集**，不是顶层结构。简单场景可直接套句式，复杂多分镜的多模态参考必须以三段论为骨架。路径分流只决定**内容层形态**（一段式 vs 三段论），不豁免交付层——两条路径最终都要过 Step 4.0 的 12 项要素覆盖清单（形式自由，覆盖即可）。
...
- 路径 B 仍负责总体设定 + 镜头分镜 + 风格约束三段论，不改变原有镜头顺序和主体绑定。
...
适用于单镜 / 一两句话能描述清楚的请求，包括 **多模态参考、编辑、延长、组合** 全部 4 类任务。直接组装为一段连续任务内容（对照 Step 4.0 的要素清单自查覆盖即可，无需套字段标题）；**不要**把简单动作强拆成"镜头 1 / 镜头 2"：
...
## 强制约束（总览）
- **任务类型优先 → 多模态参考再看复杂度**：编辑 / 延长 / 组合任务一律走路径 A；多模态参考任务按复杂度走路径 A（一段式）或路径 B（三段论）。路径 A 允许跳过 `seedance-cinematic-format`，但只减少结构开销，不降低交付要求；两条路径最终都要覆盖 Step 4.0 的 12 项要素、八大核心要素与 Step 4.1 五大内容块（要素可增不可减，标题形式自由）。
