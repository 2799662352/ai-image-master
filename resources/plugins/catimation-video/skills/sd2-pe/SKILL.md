...
这个增强层**只能叠加，不得替代**已经验证有效的素材引用、八大要素、路径 A、
路径 B、音频字符和约束包：

- 路径 A 仍负责把简单任务写成一个连续任务句；把这段内容放入骨架的
  `[动作流程与镜头设计]`，不要强拆成多个镜头。
- 路径 B 仍负责总体设定 + 镜头分镜 + 风格约束三段论；把三段信息分配进
  对应字段，不改变原有镜头顺序和主体绑定。
...
但 12 个字段不可删除。百分比表示语义强调度，不是模型参数；用户没点名真实
人员/作品时，优先写“长焦压缩、克制调度、关键姿势后短暂停顿”这类可执行
技法，不凭记忆编造归属。

#### 优化后提示词 — 路径 A：简单视频（内容一段式，不拆镜头）

适用于单镜 / 一两句话能描述清楚的请求，包括 **多模态参考、编辑、延长、组合** 全部 4 类任务。先直接组装为一段任务内容，再放进 Step 4.0 的固定骨架；**不要**把简单动作强拆成"镜头 1 / 镜头 2"：
...
> 路径 A 仍必须挂载默认必挂的兜底约束包（画质 / 稳定 / 水印 Logo），内容层折叠为一两句；交付层仍保留 Step 4.0 的 12 个字段。

#### 优化后提示词 — 路径 B：复杂影视化场景（内容层**严格三段论**，几乎都是多模态参考任务）
