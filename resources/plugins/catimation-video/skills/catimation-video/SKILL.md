...
> 两个底座上一字不改地成立,换底座不换纪律。
>
> ⚠️ 但**公式不通用**:「八大要素 / 12 项内容 / 五大必备块」是 Seedance 的写法,
> 万相有自己的一套(六段 30 秒时间轴 + 主体·场景·运动·美学控制·风格化·音效)。
> 别把两套套在同一条提示词上 —— 那不是更完整,是自相矛盾。

**路径 A / B 与结构叶子只属于 Seedance 那两个底座。**
...
> **`model` 是 `wan3` 时不走这套,也不要加载 seedance-cinematic-format。**
> 万相的结构由 wan3-cinematic-30s 自带(30 秒六段时间轴),复杂题材由它按产品 /
> 情绪对白引出专项叶子。把 Seedance 的结构叶子叠上去,等于给一条万相提示词套两套
> 互不相容的骨架。
>
> 同理,**名字带 `seedance-` 的技法叶子都是 Seedance 专属**(模型对齐、配额、
> 爆款体检那类),万相任务不载 —— 它们给的是另一个上游的参数建议。与模型无关的
> 技法(表演、光位、连续性、物理、负向控制)两边通用,照常按症状挑。
...
下面第 1 条是 Seedance 那两个底座的流程。**`model` 是 `wan3` 时跳过它**,改用
wan3-cinematic-30s 的 30 秒六段结构;第 2 条起的技法、资产、QA 纪律两边通用。

Seedance 侧:快速模式可以走路径 A 并跳过结构叶子,但八大要素、12 项覆盖与五大
必备内容块仍不得缺失。

1. **`sd2-pe`(Seedance 必经底座) + seedance-cinematic-format(复杂任务条件加载)**:
