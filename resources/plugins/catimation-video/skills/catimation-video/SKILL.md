...
轻量连续任务可由 `sd2-pe` 独立完成；路径 B、多镜、混合媒介或需要展开导演/
作品参考时再加载结构叶子 seedance-cinematic-format。下文「对症加载 /
别自行放大」同样适用于该结构叶子。
...
1. **`sd2-pe`(必经底座) + seedance-cinematic-format(复杂任务条件加载)**:
   路径 A 由底座直接完成连续任务正文;路径 B、多镜、混合媒介或展开导演/作品
   参考时加载结构叶子。两条路径都覆盖八大要素、12 项内容和五大必备块,只是
   表达结构不同;并主动出 2–3 个已核实影视参考候选供用户挑选。真人、2D 动画、
   3D 动画作为可组合
