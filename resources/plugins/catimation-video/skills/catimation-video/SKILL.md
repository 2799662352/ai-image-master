...
Seedance 语法/全能参考问答——**自动加载提示词底座**,不需用户点名。

> **底座是两个,同级,一起载:`sd2-pe` + `sd25-pe`。** 不是二选一 —— 前者管引用语法、
> 八大要素、镜头分镜的通用工程写法;后者是火山方舟官方对 2.5 的定义,管编辑 / 延长 /
> 关键帧 / 宫格分镜 / 白模、素材逐份职责、30 秒时间段。两个都不占技法名额。
>
> **默认按 2.5 写。** 本 app 的视频链路以 Seedance 2.5 为准,它是唯一支持 `taskMode`
> 编辑 / 延长、30 秒时长和 50 份素材的档位;模型未知时按它写不会亏。只有卡片 / 请求的
> model **明确**是 2.0 家族(`2.0` / `2.0-fast` / `2.0-mini`)时,才回到 2.0 的八大要素
> 公式与 15 秒上限。
>
> **同级不等于同时套模板** —— 套模板只套一套,别把两套揉进同一条 Prompt。下文凡写
> `sd2-pe` 的纪律(绑定语法、多镜拆卡、资产门)在 2.5 上一字不改地成立。

路径 A
仅限简单、单镜的轻量连续任务，可由底座独立完成；复杂、多镜、混合媒介或
需要展开导演/作品参考任一命中即进入路径 B，并加载结构叶子
seedance-cinematic-format。路径 B 条件优先于生成/编辑/延长/组合等任务类型。
下文「对症加载 /
别自行放大」同样适用于该结构叶子。

**分镜底座自动触发(第二个例外):** 凡是**产出会跨镜/跨图连续**的任务——不止一个
镜头、一板卡片、一个系列、同一角色跨多张图,或用户给了剧本/分场/镜头清单——
**自动加载 `shotlist-builder`**,同样不需用户点名「分镜表」。它管镜头行怎么切、
哪几行合成一条 4–15s 提示词、要不要落成一张可搜索的 HTML 总表;**没有剧本时它负责
主动问出镜头表**,而不是等用户递本子。它只做规划与交付物,生成仍回本入口。
单镜「让这张图动起来」不加载。
