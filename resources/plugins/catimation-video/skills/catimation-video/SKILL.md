...
- **一张卡 = 一次生成 = 一个镜头。** 把「镜头1…镜头2…镜头3」写进同一张卡的提示词,
  模型会试图把它们压进 5–15 秒里,结果四个镜头一个都不成立。多镜拆成多张卡,
  卡的顺序就是镜头顺序。
- **下面那道「角色片/多镜」硬门在工作台上一字不改地适用。** 建卡 ≠ 已经备齐资产:
  `video_workbench_add_tasks` 默认只填不跑,正好是补锚点、排故事板、清点资产的窗口;
  `video_workbench_start` 之前这三样必须已经完成。工作台天生就是多镜场景,
  它是这道门最该生效的地方,不是例外。
...
- **用户手里已有剧本 / 分镜表 / 制片包**要一次铺满工作台时,加载 storyboard-to-workbench
  取字段映射与整板落地口径(它是叶子,只给映射与纪律,不反过来重跑本入口的分级)。
  <!-- 这里刻意不用反引号:反引号引用会计入本入口的 fanout,而 pro 预算(≤5)已被
       sd2-pe / director-orchestrator / film-studio / catimation-understand / ffmpeg-win
       占满。纯文本提及同样能让模型找到它,且不动架构预算。 -->
