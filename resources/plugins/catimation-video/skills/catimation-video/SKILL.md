...
**预算数的是你自己挑的对症技法,不是条件强制项。** 入口自身、`sd2-pe` 底座、
路径 B 的结构叶子、用户所在界面的叶子(工作台)、方向开放时的 catimation-brainstorm、
以及出图/QA/后期各自的入口与工具,都是命中条件才载入、不由你自由取舍,不占这个额度。
额度约束的是「按风险挑几个技法」——那才是容易失控的地方。
