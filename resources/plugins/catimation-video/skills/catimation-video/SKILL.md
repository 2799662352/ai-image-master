...
> **底座两个同级,但按 model 只载一个。** `sd2-pe` 与 `sd25-pe` 在路由表里平起平坐,
> 「同级」说的是地位,不是要同时塞进上下文 —— 两份加起来约 70KB,每个视频任务都
...
> 只在真要写 2.0 的八大要素公式时才去读那一份。两个都不占技法名额。
>
> 下文凡写 `sd2-pe` 的纪律(绑定语法、多镜拆卡、资产门)在 2.5 上一字不改地成立,
> 换底座不换纪律。
