...
### 0. まず `director-orchestrator` の STEP 0 を実行(画幅/尺/13 維の反問)

**最初の一手は反問。`director-orchestrator` を Read し、その STEP 0(画幅・尺・13 維の自反問)をこのタスクに対して声に出して答え、ユーザーにも一行で見せてから 1→4 に進む。反問を飛ばして直接 prompt を書くのは禁止。**
...
**まず映像の「見方」:** `view_image` で MP4 は直接開けない——**画作りの視覚チェック**は `ffmpeg-win` で 3×3 / 4×4 の九宮格 contact sheet を抽出して `view_image`、**ストーリー / 連続性の内容理解**は `catimation-understand` の `understand_video`(モデルが代わりに通覧)を使う。**この二つは同じ自己点検の両面——どちらか一方で済ませず両方を組み合わせる**。MP4 の生バイトはチャットに投げない。NG ならリテイク点を添えて再生成。**絵コンテ・キーポーズ・尾帧は捨てずに再利用**——Seedance i2v の `referenceImages` / `firstFrame` の一級素材になる。
