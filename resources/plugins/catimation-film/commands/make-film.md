...
- **G5 图生视频:每条镜头提示词都必须先经 `sd2-pe`(视频与提示词的总兜底/起点)结构化**(八大要素 + 统一三段结构 + 兜底包 + 必载结构叶子 seedance-cinematic-format 的 12 字段骨架输出),再交 `generate_video`;成套分镜先用 director-orchestrator 做 13 维定位,落地仍回到 sd2-pe。
