...
- **G5 图生视频:每条镜头提示词都必须先经 `sd2-pe`(视频与提示词的总兜底/起点)结构化**(八大要素 + 12 项内容 + 五大必备块 + 兜底包)。整片/多镜属于路径 B,加载 `seedance-cinematic-format`,但标题与分段形式自由,再交 `generate_video`;成套分镜先用 director-orchestrator 做 13 维定位,落地仍回到 sd2-pe。
